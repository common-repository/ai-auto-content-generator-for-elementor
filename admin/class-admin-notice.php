 ...
 *   aacgfe_pro_create_admin_notice( array('id'=>'bp-greeting-mesage','message'=>'Hey there!') );
 * 
 *              create a admin text error message
 * aacgfe_pro_create_admin_notice( array('id'=>'bp-error-mesage','message'=>'this is an example of error!','type'=>'error') );
 ...
 * aacgfe_pro_create_admin_notice( 
 *          array(
 ...
 ***************************************************************************************************** 
 */
if (!class_exists('aacgfe_admin_notices')):

    final class aacgfe_admin_notices
    {
         ...
         */
        public static function aacgfe_create_notice()
        {
         ...
         *
         */
        public function aacgfe_add_message($notice)
        {
            if( !isset( $notice['id']) || empty($notice['id']) ){
                $this->aacgfe_show_error('id is required for integrating admin notice.');
                return;
            }
            ...
            if ( isset($notice['review']) && true != (bool)$notice['review'] && ( !isset($notice['message']) || empty($notice['message']) )) {
                $this->aacgfe_show_error('message can not be null. You must provide some text for message field');
                return;
            }
            ...
            if( $review == true && ( empty( $slug ) || empty( $plugin_name ) || empty( $review_url ) )){
                $this->aacgfe_show_error( 'slug / plugin_name / review_url can not be empty if admin notice is set to review' );
                return;
            }
            ...
            add_action('admin_notices', array($this, 'aacgfe_show_notice'));
            add_action( 'admin_print_scripts', array($this, 'aacgfe_load_script' ) );
            add_action('wp_ajax_aacgfe_admin_notice', array($this, 'aacgfe_admin_notice_dismiss'));
            add_action('wp_ajax_aacgfe_admin_review_notice_dismiss', array($this, 'aacgfe_admin_review_notice_dismiss'));
        }
    	 ...
    	 * @return void
    	 */
    	public function aacgfe_load_script() {    	
            wp_register_style( 'aacgfe-review-css', AACGFE_URL . 'assets/css/aacgfe-admin-notice.css', null, null, 'all' );
            wp_enqueue_style( 'aacgfe-review-css');
         ...
         */
        public function aacgfe_show_notice()
        {
            if (count($this->messages) > 0) {
                
                foreach ($this->messages as $id => $message) {
                    if( true == (bool) $message['review'] ){
                        $this->aacgfe_admin_notice_for_review( $id, $message);
                    }else{
                        $this->aacgfe_simple_notice($id, $message );
                    }
                }
            }
        }
         ...
         */
        private function aacgfe_simple_notice($id, $message ){
            ...
            jQuery(document).ready(function ($) {
                ...
                $(document).on("click",".'.$id.'_admin_notice button.notice-dismiss", function (event) {
                    ...
                    $.post(ajaxURL, { "action":"aacgfe_admin_notice","id":id,"_nonce":wp_nonce }, function( data ) {
            ...
            echo "<div class='".$id."_admin_notice $classes aacgfe-simple-notice' data-ajax-url='".admin_url('admin-ajax.php')."' data-wp-nonce='". $nonce . "' data-plugin-slug='$id'>$image_html<div class='message_container'><p>" . $message['message'] . "</p></div></div>" . $script;
        }
         ...
         */
        private function aacgfe_admin_notice_for_review( $id, $messageObj ){
            ...
            if(get_option( 'aacgfe-installDate' )){
                // get installation dates and rated settings
                //$installation_date =date( 'Y-m-d h:i:s', get_option( 'ect-installDate' ));
                $installation_date =date( 'Y-m-d h:i:s', strtotime(get_option( 'aacgfe-installDate' )) );
            }else{
                $this->aacgfe_show_error('Review notice can not be integrated. aacgfe-installDate option is not set for the plugin');
                return;
            }
                       
               
                $alreadyRated =get_option( 'aacgfe-ratingDiv' )!=false?get_option( 'aacgfe-ratingDiv'):"no";
              ...
              if (isset($diff_days) && $diff_days>= $days ) {
                    echo $this->aacgfe_create_notice_content( $id, $messageObj );
                }
        }
         ...
         **/ 
       function aacgfe_create_notice_content( $id, $messageObj ){

        $ajax_url=admin_url( 'admin-ajax.php' );
        $ajax_callback = 'aacgfe_admin_review_notice_dismiss';
        ...
        */
        public function aacgfe_admin_review_notice_dismiss(){
            ...
            if ( ! check_ajax_referer($nonce_key,'_nonce', false ) ) {
                ...
                update_option( 'aacgfe-ratingDiv','yes' );
         ...
         * This is called by a wordpress ajax hook                  *
         ************************************************************/
        public function aacgfe_admin_notice_dismiss()
        {
         ...
         **************************************************************/
        private function aacgfe_show_error($error_text){
    ...
    function aacgfe_create_admin_notice($notice)
    {
        ...
        $main_class = aacgfe_admin_notices::aacgfe_create_notice();
        $main_class->aacgfe_add_message($notice);
        return $main_class;
    }